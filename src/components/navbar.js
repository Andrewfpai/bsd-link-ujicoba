...
function Navbar({sendDataToHome}) {
  ...
  return (
    ...
    <div className="flex flex-row items-center justify-between pt-5 pb-4">
        <img src={Logo} alt='logo' className='w-44 cursor-pointer ml-5' onClick={()=>{navigate('/')}} />
        <ul className="hidden desktop:flex flex-row justify-center gap-10 text-darkGrayishBlue">
           
            <Link to={'/aboutBSDLink'} className='hover:text-softRed cursor-pointer'>About BSD Link</Link>
            <Link to={'/aboutUs'} className='hover:text-softRed cursor-pointer'>About Us</Link>
            <Link to={'/help'}className='hover:text-softRed cursor-pointer'>Help</Link>
        </ul>
